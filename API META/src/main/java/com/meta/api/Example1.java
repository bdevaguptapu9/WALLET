       ...
       if (200 == response.statusCode()) {
    	 ...
    	 //   org.jsoup.nodes.Element element2 = doc.head().select("link[href~=.*\\.(ico|png)]").first();
    	  ...
    	  // org.jsoup.nodes.Element metaOgImage = doc.select("meta[property=og:image]").first();
    	   ...
    	   //keywords
    	   Elements key = doc.select("meta[name=keywords]");
    	   String tempkey = key.attr("content");
    	   if(tempkey !=""){
    		     keywords   = tempkey;
    	                   }
    	   else{
    		   Elements metaOgkey = doc.select("meta[property=og:keywords]");
    		   keywords = metaOgkey.attr("content");
    		   if(keywords == ""){
    			   metaOgkey = doc.select("meta[name=twitter:keywords]");
    			   keywords = metaOgkey.attr("content");
    			   if(keywords ==""){
    				   metaOgkey = doc.select("meta[itemprop=keywords]");
    				   keywords =  metaOgkey.attr("content");
    			                }
    		                 }
        ...
        ObjectMapper mapper = new ObjectMapper();
	    System.out.println(mapper.writeValueAsString(main));
	    fmeta.setGENJSON(mapper.writeValueAsString(main));
		return mapper.writeValueAsString(main);
